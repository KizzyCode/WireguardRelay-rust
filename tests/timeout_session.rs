...
const WGPROXY_PORTS: RangeInclusive<u16> = 60200..=60299;

#[test]
pub fn session_timeout() {
    /// The timeout for an existing session
    const WGPROXY_TIMEOUT: Duration = Duration::from_secs(3);
    ...
    let handshake0 = utils::handshake(&WGPROXY_PUBKEY, 0);
    let handshake1 = utils::handshake(&WGPROXY_PUBKEY, 1);
    let mut buf = [0; 512];
    ...
    client.send_to(&handshake0, wgproxy_listen).expect("failed to send test packet");
    ...
    thread::sleep(WGPROXY_TIMEOUT + Duration::from_secs(3));
    server.send_to(b"testolope:0", relay_nat_address).expect("failed to send test reply");

    // Send another packet to the server
    client.send_to(&handshake1, wgproxy_listen).expect("failed to send test packet");
    let (buf_len, relay_nat_address) = server.recv_from(&mut buf).expect("failed to receive test packet");
