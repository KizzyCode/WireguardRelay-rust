...
fn all_tests() {
    ...
    let wgproxy = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), WGPROXY_PORTS.skip(0).next().unwrap());
    let config = Config {
    ...
    // Execute tests
    dbg!(session::simple_routing(&server, &wgproxy, &config));
    dbg!(session::interleaved_routing(&server, &wgproxy, &config));
